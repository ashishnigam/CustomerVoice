...
- Local infra: Docker Compose (postgres, redis, mailhog, minio, analytics-postgres, superset)
...
LCOV output is now generated by the API Vitest suite.
...
Full run including DB-backed integration tests:
...
CustomerVoice now supports OpenTelemetry tracing in the API and worker services.
...
- `OTEL_EXPORTER_OTLP_ENDPOINT=` for an optional external collector

When analytics storage is configured, spans are written to the `otel_spans` table in the analytics Postgres instance. Trace records include tenant, workspace, request, impersonation, and worker job metadata so Superset or a downstream warehouse can segment telemetry per tenant.

## Superset

Start the analytics stack with:

```bash
pnpm infra:analytics:up
```

Then open `http://localhost:8088` and sign in with:

- Username: `admin`
- Password: `admin`

To register the analytics database inside Superset, use:

```text
postgresql+psycopg2://analytics:analytics@analytics-postgres:5432/customervoice_observability
```

The longer observability setup notes are in `/Users/ashishnigam/Startups/CustomerVoice/docs/CustomerVoice-Observability-Runbook.md`.
